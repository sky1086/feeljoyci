...
function initCardPage(hash, isBasePage, data, ifClose, openMenu) {
  ...
  for(i = 0; i < data.length; i++) {
    ...
    if(isBasePage) {
      ...
      colorClass = 'white';
      doClasses(button, ['waves-effect', 'waves-dark', 'fj-card-figure', 'z-depth-1', colorClass]);
    }
...
function dotSplit(value) {
  ...
  for(i = 0; i < value.length; i++) {
    current = current.concat(value[i]);
    // if above char limit, check if it's a space or not
    if(i > char_limit && value[i] === ' ') {
      isBreak = true;
      char_limit += 140;
    }
    // break this into a new card
    if((isBreak || (i + 1) === value.length) && current.trim()
      .length > 0) {
