...
var email = document.getElementById('email'),
  ...
  fjInitialLogin = document.getElementById('fj-initial-login'),
  fjInitialCreation = document.getElementById('fj-initial-creation'),
  fjGEmail = document.getElementById('fj-g-email'),
  fjNextEmail = document.getElementById('fj-next-email'),
  fjPasswordButton = document.getElementById('fj-password-button'),
  fjMagicButton = document.getElementById('fj-magic-button'),
  fjPasswordLogin = document.getElementById('fj-password-login'),
  true_dark_colors_hashes = ['#0097A7', '#00796B', '#388E3C', '#689F38'],
  i,
  isLogin = false,
  curPage = 0,
  welcomePages = [],
...
// add pages to vars
for(var i = 1; i < 5; i++) {
  welcomePages.push(document.getElementById('fj-welcome-card-' + i));
}
...
// https://hackernoon.com/how-to-reduce-incorrect-email-addresses-df3b70cb15a9#.blshdv2ta
function checkForCloseMatch(longString, shortString) {
  ...
  for(var i = 1; i < shortString.length; i++) {
    ...
    if(secondPart !== 'mail') {
      var missingLetterRegEx = new RegExp(firstPart + '{0}' + secondPart);
    ...
    var switchedLetters = [shortString.substring(0, i - 1), shortString.charAt(i), shortString.charAt(i - 1), shortString.substring(i + 1)].join('');
...
function checkForDomainTypo(userEmail) {
  var domains = ['gmail', 'hotmail', 'outlook', 'yahoo', 'icloud', 'mail', 'zoho'];

  var _userEmail$split = userEmail.split('@'),
    _userEmail$split2 = _slicedToArray(_userEmail$split, 2),
    leftPart = _userEmail$split2[0],
    rightPart = _userEmail$split2[1];

  for(var i = 0; i < domains.length; i++) {
    var domain = domains[i];

    var result = checkForCloseMatch(rightPart, domain);
...
function checkForNameTypo(userEmail, name) {
  var _userEmail$split3 = userEmail.split('@'),
    _userEmail$split4 = _slicedToArray(_userEmail$split3, 2),
    leftPart = _userEmail$split4[0],
    rightPart = _userEmail$split4[1];

  var result = checkForCloseMatch(leftPart, name);
...
function checkForCommonTypos(userInput) {
  var commonTypos = [{
    pattern: /,com$/,
    fix: function fix(str) {
      return str.replace(/,com$/, '.com');
    }
  }, {
    pattern: /,co\.\w{2}$/,
    fix: function fix(str) {
      return str.replace(/,(co\.\w{2}$)/, '.$1');
    }
  }, {
    pattern: /@\w*$/,
    fix: function fix(str) {
  ...
  typo = commonTypos.find(function (typo) {
    return typo.pattern.test(userInput);
  });
...
function checkForTypo(userInput) {
  var email = userInput.email.trim()
    .toLowerCase();

  return checkForCommonTypos(email) || checkForDomainTypo(email) || checkForNameTypo(email, userInput.firstName.trim()
    .toLowerCase()) || checkForNameTypo(email, userInput.lastName.trim()
    .toLowerCase());
}
...
// quick next page
function nextPage() {
  doClasses(welcomePages[curPage++], ['animate', 'animate-slow', 'slideOutLeft']);
  // update appbar title color
  updateThemeColor(true_dark_colors_hashes[curPage]);
}

// tap overlay to close the diversion
fjLayout_Hammer.on('tap', function (ev) {
  // save it to the site
  if(fjInitialLogin.contains(ev.target)) {
    nextPage();
    fjPasswordLogin.innerHTML = 'Login';
    isLogin = true;
    return;
  }

  if(fjInitialCreation.contains(ev.target)) {
    nextPage();
    fjPasswordLogin.innerHTML = 'Signup';
    isLogin = false;
    return;
  }

  if(fjNextEmail.contains(ev.target)) {
    nextPage();
    return;
  }

  if(fjPasswordButton.contains(ev.target)) {
    nextPage();
    return;
  }
