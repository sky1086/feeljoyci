...
class Thirdclick extends CI_Controller{
	...
	public function listing($id = 0){
		...
		$this->load->view('admin/thirdclicklist_view', $data);
	}
	...
	public function getThirdClicks($cid = 0){
		...
		$data = $this->question_model->getAssocQuestionDetails($cid);
		$result = [];
		$showAnsDiv = '';
		foreach ($data as $row){
			$title = empty($row->question)?'-N-A-':$row->question;
			$link = '<a href="'.base_url().'admin/thirdclick/index/'.$row->id.'" onclick="window.location=this.href;"><i class="ion-edit" style="font-size:18px;"></i>&nbsp;Edit</a>';
